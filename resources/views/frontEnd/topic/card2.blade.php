        ...
        <div class="video-container position-relative">
            @if ($Post->photo_file != "")
                <img class="card-img-top"
                     src="{{ URL::to('public/uploads/topics/'.$Post->photo_file) }}" loading="lazy"  width="100%" height="100%"
                     alt="{{ $post_title }}"/>
            @else
                <?php
                $img_url = "";
        ...
        <div class="audio-container position-relative">
            @if($Post->photo_file !="")
                <img class="card-img-top" loading="lazy" width="100%" height="100%"
                     src="{{ URL::to('public/uploads/topics/'.$Post->photo_file) }}"
                     alt="{{ $post_title }}"/>
            @endif
            <div class="audio-player">
                <audio crossorigin preload="none">
                    <source
                        src="{{ URL::to('public/uploads/topics/'.$Post->audio_file) }}"
                        type="audio/mpeg">
                </audio>
            </div>
        </div>
        ...
        <div class="image-container position-relative">
            ...
            @else
                @foreach($Post->photos->random(1) as $photo)
                    <img class="card-img-top" loading="lazy"  width="100%" height="100%"
                         src="{{ URL::to('public/uploads/topics/'.$photo->file) }}"
                         alt="{{ $post_title }}"/>
                @endforeach
            @endif
