...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('public/assets/dashboard/css/print.css') }}" type="text/css"/>
</head>
...
@if(count($Topics) >0)
                    ...
                    if (in_array(Auth::user()->permissions_id, $view_permission_groups) || in_array(0, $view_permission_groups) || $customField->view_permission_groups == "") {
                        ...
                        if (($cf_saved_val != "" || count($cf_saved_val_array) > 0) && ($customField->lang_code == "all" || $customField->lang_code == @Helper::currentLanguage()->code)) {
                            if ($customField->type == 12) {
                                ...
                                $cf_data = "<a target='_blank' href='" . URL::to('public/uploads/topics/' . $cf_saved_val) . "'><i class='fa fa-play'></i></a>";
                            } elseif ($customField->type == 9) {
                                $cf_data = "<a target='_blank' href='" . URL::to('public/uploads/topics/' . $cf_saved_val) . "'><i class='fa fa-play'></i></a>";
                            } elseif ($customField->type == 8) {
                                $cf_data = "<a target='_blank' href='" . URL::to('public/uploads/topics/' . $cf_saved_val) . "'><i class='fa fa-picture-o'></i></a>";
                ...
                <tr>
                    @foreach($Cols as $KEY=>$COL)
                        ...
                        if ($KEY == "col_id") {
                            ...
                            $photo = "";
                            if ($Topic->photo_file != "") {
                                $photo = " <div class=\"pull-right\"><img src=\"" . asset('public/uploads/topics/' . $Topic->photo_file) . "\" style=\"height: 40px\" alt=\"" . $title . "\"></div>";
                            }
    ...
    @if(@$stat == "excel")
        <script src="{{ asset('public/assets/dashboard/js/xlsx.full.min.js') }}"></script>
